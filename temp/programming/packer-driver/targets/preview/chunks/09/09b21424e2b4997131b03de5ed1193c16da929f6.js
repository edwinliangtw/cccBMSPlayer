      ...
      _export("BMSParser", BMSParser = class BMSParser {
        constructor() {
          ...
          this._expand = function (array, length) {
            ...
            var interval = length / array.length;
            var results = [];
          ...
          this._merge = function (ary1, ary2) {
            ...
            var lcm = this._lcm(ary1.length, ary2.length);

            var ret = this._expand(ary1, lcm);

            var ref = this._expand(ary2, lcm);
        ...
        _parse(row) {
          ...
          var channelMsg = /^#([0-9]{3})([0-9]{2}):([\w\.]+)/.exec(row);
        ...
        _parseChannelMsg(msg) {
          // 第幾小節
          var measureNum = parseInt(msg[1]); // 第幾channel

          var ch = parseInt(msg[2]); // channel資料

          var data = msg[3]; // 建立空資料結構
        ...
        // 解析整體後修復時間等
        _modifyAfterParse() {
          var bpm = parseFloat(this.bms.bpm);
          var data = this.bms.data;
          var time = 0;
          var results = [];
          ...
          for (var i = 0, dl = data.length; i < dl; i++) {
            ...
            for (var j = 0, ml = bar.bpm.message.length; j < ml; j++) {
              var val = bar.bpm.message[j];
        ...
        _calcTiming(time, objects, bpmobj, bpm, meter) {
          var bl = bpmobj.message.length;
          var ol = objects.message.length;

          var lcm = this._lcm(bl, ol);

          var bpms = this._expand(bpmobj.message, lcm);

          var objs = this._expand(objects.message, lcm);
          ...
          for (var i = 0, l = bpms.length; i < l; i++) {
            var val = bpms[i];
        ...
        _noteTiming(time, bar, bpm) {
          for (var i = 0, l = bar.note.key.length; i < l; i++) {
            var key = bar.note.key[i];
        ...
        _wavTiming(time, bar, bpm, wavss) {
          ...
          for (var i = 0, wl = wavss.length; i < wl; i++) {
            var ws = wavss[i];

            var lcm = this._lcm(bar.bpm.message.length, ws.length);

            var bpms = this._expand(bar.bpm.message, lcm);

            var wavs = this._expand(ws, lcm);

            var t = 0;
            var b = bpm;

            for (var j = 0, bl = bpms.length; j < bl; j++) {
              var val = bpms[j];
          ...
          var sorted = result.sort((a, b) => a.timing - b.timing);
          var results = [];

          for (var k = 0, sl = sorted.length; k < sl; k++) {
            var w = sorted[k];
        ...
        _serialize(arr, name, bms_data) {
          ...
          var _loop = function _loop(i, bl) {
            var bmsData = bms_data[i];
            serialized.push(function () {
              var timing = bmsData[name].timing;
              var results = [];

              for (var j = 0, _len1 = timing.length; j < _len1; j++) {
                var t = timing[j];
          ...
          for (var i = 0, bl = bms_data.length; i < bl; i++) {
            _loop(i, bl);
          }
...
//# sourceMappingURL=09b21424e2b4997131b03de5ed1193c16da929f6.js.map